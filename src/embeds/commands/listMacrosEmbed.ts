...
export default async function listMacrosEmbed(guild: Guild, macros: Macro[]) {
    const premium = await gb.database.getPremium(guild.id);
    return new RichEmbed()
        .setColor(`#a8ff6c`)
        .addField(`Macros for ${guild}`, macros.length
            ? macros.map(macro => `\`${macro.macro_name}\``).join(', ')
            : `${gb.emojis.get('hifumi_wew')} this place is so empty. Make a new macro with \`${await gb.database.getPrefix(guild.id)}addmacro\`!`)
