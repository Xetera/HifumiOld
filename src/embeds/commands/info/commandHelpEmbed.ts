...
import {formatAndJoin, formattedTimeString} from "../../../utility/Util";
...
export default function commandHelpEmbed(command: Command, prefix: string, guild: Guild){
    ...
    if (command.clientPermissions && command.clientPermissions.length){
        description += (`**I need permissions:** ${formatAndJoin(command.clientPermissions.map(c => c.toString()))}\n`)
    }
    if (command.userPermissions && command.userPermissions){
        description += (`**User needs permission:** \`${command.userPermissions.toString()}\`\n`)
    }
