...
export default async function setInvites(message: Message, input: [boolean]){
    ...
    const allowed  = await gb.database.getAllowGuildInvites(message.guild.id);
    ...
    gb.database.setAllowGuildInvites(message.guild.id, inputAllowInvites).then(async (response: Partial<Guild>)=> {
